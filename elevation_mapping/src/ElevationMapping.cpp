...
ElevationMapping::ElevationMapping(ros::NodeHandle& nodeHandle,
  ...
  switch (sensorType) {
    case PRIME_SENSE:
      sensorProcessor_.reset(new PrimeSenseSensorProcessor(transformListener_));
      break;
    case ASLAM:
      sensorProcessor_.reset(new PrimeSenseSensorProcessor(transformListener_));
      break;
  ...
  pointCloudSubscriber_ = nodeHandle_.subscribe(
      pointCloudTopic_, 1, &ElevationMapping::pointCloudCallback, this);
  elevationMapRawPublisher_ = nodeHandle_
      .advertise<elevation_map_msg::ElevationMap>("elevation_map_raw", 1);
  elevationMapPublisher_ = nodeHandle_
      .advertise<elevation_map_msg::ElevationMap>("elevation_map", 1);
  robotPoseSubscriber_.subscribe(nodeHandle_, robotPoseTopic_, 1);
  robotPoseCache_.connectInput(robotPoseSubscriber_);
  robotPoseCache_.setCacheSize(robotPoseCacheSize_);
  mapUpdateTimer_ = nodeHandle_.createTimer(
      maxNoUpdateDuration_, &ElevationMapping::mapUpdateTimerCallback, this,
      true, false);
  fusionTriggerService_ = nodeHandle_.advertiseService(
      "trigger_fusion", &ElevationMapping::fuseEntireMap, this);
  submapService_ = nodeHandle_.advertiseService("get_submap",
                                                &ElevationMapping::getSubmap,
                                                this);
  initialize();
}
...
bool ElevationMapping::readParameters() {
  ...
  nodeHandle_.param("robot_pose_topic", robotPoseTopic_,
                    string("/robot_state/pose"));
  nodeHandle_.param("parent_frame_id", parentFrameId_, string("/map"));
  nodeHandle_.param("track_point_frame_id", trackPointFrameId_,
                    string("/robot"));
  ...
  nodeHandle_.param("elevation_map_frame_id", map_.frameId_,
                    string("/elevation_map"));
  ...
  nodeHandle_.param("mahalanobis_distance_threshold",
                    map_.mahalanobisDistanceThreshold_, 2.5);
  nodeHandle_.param("multi_height_noise", map_.multiHeightNoise_,
                    pow(0.003, 2));
  nodeHandle_.param("min_horizontal_variance", map_.minHorizontalVariance_,
                    pow(resolution / 2.0, 2));  // two-sigma
  nodeHandle_.param("max_horizontal_variance", map_.maxHorizontalVariance_,
                    0.5);
  ...
  for (std::size_t i = 0; i < nParameters; i++)
    nodeHandle_.param(sensorProcessor_->sensorParameterNames_[i].c_str(),
                      sensorProcessor_->sensorParameters_[i], 0.0);

  nodeHandle_.param("base_frame_id", sensorProcessor_->baseFrameId_,
                    string("/robot"));

  ROS_ASSERT(sensorProcessor_->sensorParameters_[0] >= 0.0);
  ROS_ASSERT(
      sensorProcessor_->sensorParameters_[1]
          > sensorProcessor_->sensorParameters_[0]);
...
void ElevationMapping::pointCloudCallback(
  ...
  // Update map location.
  updateMapLocation();
  if (!broadcastElevationMapTransform(time))
    ROS_ERROR(
        "ElevationMap: Broadcasting elevation map transform to parent failed.");
  ...
  if (!poseMessage) {
    ROS_ERROR(
        "Could not get pose information from robot for time %f. Buffer empty?",
        time.toSec());
    return;
  }

  Matrix<double, 6, 6> robotPoseCovariance = Map<const MatrixXd>(
  ...
  if (!publishRawElevationMap())
    ROS_INFO("ElevationMap: Elevation map has not been broadcasted.");
...
void ElevationMapping::mapUpdateTimerCallback(
  ...
  // Publish raw elevation map.
  if (!publishRawElevationMap())
    ROS_ERROR("ElevationMap: Elevation map has not been broadcasted.");

  resetMapUpdateTimer();
}

bool ElevationMapping::fuseEntireMap(std_srvs::Empty::Request& request,
...
bool ElevationMapping::broadcastElevationMapTransform(const ros::Time& time) {
  ...
  transformBroadcaster_.sendTransform(
      tf::StampedTransform(tfTransform, time, parentFrameId_, map_.frameId_));
  ROS_DEBUG("Published transform for elevation map in parent frame at time %f.",
            time.toSec());
  return true;
}

bool ElevationMapping::updatePrediction(const ros::Time& time) {
  ROS_DEBUG("Updating map with latest prediction from time %f.",
            robotPoseCache_.getLatestTime().toSec());

  if (time < map_.getTimeOfLastUpdate()) {
    ROS_ERROR(
        "Requested update with time stamp %f, but time of last update was %f.",
        time.toSec(), map_.getTimeOfLastUpdate().toSec());
    return false;
  }
  ...
  kindr::poses::eigen_impl::convertFromRosGeometryMsg(poseMessage->pose.pose,
                                                      robotPose);
  Matrix<double, 6, 6> robotPoseCovariance = Map<const MatrixXd>(
      poseMessage->pose.covariance.data(), 6, 6);
...
bool ElevationMapping::publishRawElevationMap() {
  ...
  elevation_map_msg::matrixEigenToMultiArrayMessage(
      map_.getRawElevationData(), elevationMapMessage.elevation);
  elevation_map_msg::matrixEigenToMultiArrayMessage(
      map_.getRawVarianceData(), elevationMapMessage.variance);
  elevation_map_msg::matrixEigenToMultiArrayMessage(map_.getRawColorData(),
                                                    elevationMapMessage.color);
...
bool ElevationMapping::publishElevationMap() {
  if (elevationMapPublisher_.getNumSubscribers() < 1)
    return false;
  ...
  elevation_map_msg::matrixEigenToMultiArrayMessage(
      map_.getElevationData(), elevationMapMessage.elevation);
  elevation_map_msg::matrixEigenToMultiArrayMessage(
      map_.getVarianceData(), elevationMapMessage.variance);
  elevation_map_msg::matrixEigenToMultiArrayMessage(map_.getColorData(),
                                                    elevationMapMessage.color);
...
void ElevationMapping::addHeaderDataToElevationMessage(
    elevation_map_msg::ElevationMap& elevationMapMessage) {
...
bool ElevationMapping::updateMapLocation() {
  ...
  try {
    transformListener_.transformPoint(parentFrameId_, trackPoint,
...
bool ElevationMapping::getSubmap(
    elevation_map_msg::GetSubmap::Request& request,
    elevation_map_msg::GetSubmap::Response& response) {
  // Request
  ...
  map_.getSubmap(submapElevation, submapPosition, submapLength,
                 submapBufferSize, requestedIndexInSubmap,
                 map_.getElevationData(), requestedSubmapPosition,
                 requestedSubmapLength);
  map_.getSubmap(submapVariance, submapPosition, submapLength, submapBufferSize,
                 requestedIndexInSubmap, map_.getVarianceData(),
...
//  map_.getSubmap(submapColor, submapPosition, submapLength, submapBufferSize, requestedIndexInSubmap, map_.getColorData(), requestedSubmapPosition, requestedSubmapLength);
  ...
  elevation_map_msg::matrixEigenToMultiArrayMessage(
      submapElevation, response.elevation_map.elevation);
  elevation_map_msg::matrixEigenToMultiArrayMessage(
      submapVariance, response.elevation_map.variance);

  ROS_DEBUG("Elevation submap responded with timestamp %f.",
            map_.getTimeOfLastFusion().toSec());
...
void ElevationMapping::resetMapUpdateTimer() {
  ...
  mapUpdateTimer_.setPeriod(
      maxNoUpdateDuration_ - (Time::now() - map_.getTimeOfLastUpdate()));
