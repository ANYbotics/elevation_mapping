...
bool ElevationMap::setGeometry(
  ...
  ROS_DEBUG_STREAM(
      "Elevation map matrix resized to " << elevationRawData_.rows() << " rows and " << elevationRawData_.cols() << " columns.");
  return true;
}

bool ElevationMap::add(const pcl::PointCloud<pcl::PointXYZRGB>::Ptr pointCloud,
  ...
  for (unsigned int i = 0; i < pointCloud->size(); ++i) {
    ...
    Array2i index;
    if (!elevation_map_msg::getIndexFromPosition(index,
                                                 Vector2d(point.x, point.y),
                                                 length_,
                                                 position_.vector().head(2),
                                                 resolution_, getBufferSize(),
                                                 bufferStartIndex_))
    ...
    if (mahalanobisDistance < mahalanobisDistanceThreshold_) {
      ...
      elevation = (variance * (position_.z() + point.z)
          + pointVariance * elevation) / (variance + pointVariance);
...
bool ElevationMap::update(Eigen::MatrixXf varianceUpdate,
                          Eigen::MatrixXf horizontalVarianceUpdateX,
                          Eigen::MatrixXf horizontalVarianceUpdateY,
                          const ros::Time& time) {
  Array2i bufferSize = getBufferSize();

  if (!((Array2i(varianceUpdate.rows(), varianceUpdate.cols()) == bufferSize)
      .all()
      && (Array2i(horizontalVarianceUpdateX.rows(),
                  horizontalVarianceUpdateX.cols()) == bufferSize).all()
      && (Array2i(horizontalVarianceUpdateY.rows(),
                  horizontalVarianceUpdateY.cols()) == bufferSize).all())) {
...
bool ElevationMap::fuseArea(const Eigen::Vector2d& position,
                            const Eigen::Array2d& length) {
  ROS_DEBUG(
      "Requested to fuse an area of the elevation map with center at (%f, %f) and side lenghts (%f, %f)",
  ...
  elevation_map_msg::getSubmapInformation(topLeftIndex, submapBufferSize,
                                          submapPosition, submapLength,
                                          requestedIndexInSubmap, position,
                                          length, length_,
                                          position_.vector().head(2),
                                          resolution_, getBufferSize(),
                                          bufferStartIndex_);
...
bool ElevationMap::fuse(const Eigen::Array2i& topLeftIndex,
  ...
  for (Array2i areaIndex = Array2i::Zero();
      elevation_map_msg::incrementIndexForSubmap(areaIndex, index, topLeftIndex,
                                                 size, getBufferSize(),
                                                 bufferStartIndex_);) {
    if (timer.isTiming())
      timer.stop();
    ...
    if (!std::isnan(elevationData_(i, j)))
      continue;
    ...
    Array2d requestedSubmapLength = 4
        * Array2d(horizontalVarianceRawDataX_(i, j),
                  horizontalVarianceRawDataY_(i, j)).sqrt();

    // Requested position (center) of submap in map.
    Vector2d requestedSubmapPosition;
    elevation_map_msg::getPositionFromIndex(requestedSubmapPosition,
                                            Array2i(i, j), length_,
                                            position_.vector().head(2),
                                            resolution_, getBufferSize(),
                                            bufferStartIndex_);
    ...
    // Get submap data.
    MatrixXf elevationSubmap, variancesSubmap, horizontalVariancesXSubmap,
        horizontalVariancesYSubmap;
    Vector2d submapPosition;
    Array2d submapLength;
    Array2i submapBufferSize;
    Array2i requestedIndex;

    getSubmap(elevationSubmap, submapPosition, submapLength, submapBufferSize,
              requestedIndex, elevationRawData_, requestedSubmapPosition,
              requestedSubmapLength);
    getSubmap(variancesSubmap, submapPosition, submapLength, submapBufferSize,
              requestedIndex, varianceRawData_, requestedSubmapPosition,
              requestedSubmapLength);
    getSubmap(horizontalVariancesXSubmap, submapPosition, submapLength,
              submapBufferSize, requestedIndex, horizontalVarianceRawDataX_,
              requestedSubmapPosition, requestedSubmapLength);
    getSubmap(horizontalVariancesYSubmap, submapPosition, submapLength,
              submapBufferSize, requestedIndex, horizontalVarianceRawDataY_,
              requestedSubmapPosition, requestedSubmapLength);
    ...
    elevation_map_msg::getPositionFromIndex(centerInSubmap, requestedIndex,
                                            submapLength, Vector2d::Zero(),
                                            resolution_, submapBufferSize,
                                            Array2i(0, 0));
    ...
    for (unsigned int p = 0; p < elevationSubmap.rows(); ++p) {
      for (unsigned int q = 0; q < elevationSubmap.cols(); ++q) {
        ...
        Vector2d distanceToCenter =
            (positionInSubmap - centerInSubmap).cwiseAbs();

        float probabilityX = cumulativeDistributionFunction(
            distanceToCenter.x() + resolution_ / 2.0, 0.0,
            sqrt(horizontalVariancesXSubmap(p, q)))
            - cumulativeDistributionFunction(
                distanceToCenter.x() - resolution_ / 2.0, 0.0,
                sqrt(horizontalVariancesXSubmap(p, q)));
        float probabilityY = cumulativeDistributionFunction(
            distanceToCenter.y() + resolution_ / 2.0, 0.0,
            sqrt(horizontalVariancesYSubmap(p, q)))
            - cumulativeDistributionFunction(
                distanceToCenter.y() - resolution_ / 2.0, 0.0,
                sqrt(horizontalVariancesYSubmap(p, q)));
    ...
    float variance = (weights * (variances.square() + means.square())).sum()
        / weights.sum() - pow(mean, 2);

    if (!(std::isfinite(variance) && std::isfinite(mean))) {
      ROS_ERROR(
          "Something went wrong when fusing the map: Mean = %f, Variance = %f",
          mean, variance);
      continue;
    }
  ...
  ROS_INFO("Elevation map has been fused in %f s.",
           Timing::getTotalSeconds(timerId));
  ROS_DEBUG("Mean: %f s, Min: %f s, Max: %f s.",
            Timing::getMeanSeconds(timerId), Timing::getMinSeconds(timerId),
            Timing::getMaxSeconds(timerId));
  Timing::reset(timerId);

  return true;
}

bool ElevationMap::getSubmap(Eigen::MatrixXf& submap,
  ...
  elevation_map_msg::getSubmapInformation(topLeftIndex, submapBufferSize,
                                          submapPosition, submapLength,
                                          requestedIndexInSubmap,
                                          requestedSubmapPosition,
                                          requestedSubmapLength, length_,
                                          position_.vector().head(2),
                                          resolution_, getBufferSize(),
                                          bufferStartIndex_);
  ...
  if (!elevation_map_msg::getBufferRegionsForSubmap(submapIndeces, submapSizes,
                                                    topLeftIndex,
                                                    submapBufferSize,
                                                    getBufferSize(),
                                                    bufferStartIndex_)) {
  ...
  unsigned int nRows =
      (submapSizes[3](0) + submapSizes[1](0)
          > submapSizes[2](0) + submapSizes[0](0)) ?
          submapSizes[3](0) + submapSizes[1](0) :
          submapSizes[2](0) + submapSizes[0](0);
  unsigned int nCols =
      (submapSizes[3](1) + submapSizes[2](1)
          > submapSizes[1](1) + submapSizes[0](1)) ?
          submapSizes[3](1) + submapSizes[2](1) :
          submapSizes[1](1) + submapSizes[0](1);

  submap.resize(nRows, nCols);

  submap.topLeftCorner(submapSizes[0](0), submapSizes[0](1)) = map.block(
      submapIndeces[0](0), submapIndeces[0](1), submapSizes[0](0),
      submapSizes[0](1));
  submap.topRightCorner(submapSizes[1](0), submapSizes[1](1)) = map.block(
      submapIndeces[1](0), submapIndeces[1](1), submapSizes[1](0),
      submapSizes[1](1));
  submap.bottomLeftCorner(submapSizes[2](0), submapSizes[2](1)) = map.block(
      submapIndeces[2](0), submapIndeces[2](1), submapSizes[2](0),
      submapSizes[2](1));
  submap.bottomRightCorner(submapSizes[3](0), submapSizes[3](1)) = map.block(
      submapIndeces[3](0), submapIndeces[3](1), submapSizes[3](0),
      submapSizes[3](1));

  return true;
}

bool ElevationMap::relocate(
    const kindr::phys_quant::eigen_impl::Position3D& mapFramePosition) {
  // TODO Add height shift.

  Array2i indexShift;
  Vector2d positionShift = mapFramePosition.vector().head(2)
      - pose_.getPosition().vector().head(2);
  elevation_map_msg::getIndexShiftFromPositionShift(indexShift, positionShift,
                                                    resolution_);
  Vector2d alignedPositionShift;
  elevation_map_msg::getPositionShiftFromIndexShift(alignedPositionShift,
                                                    indexShift, resolution_);
    ...
    if (indexShift[i] != 0) {
      if (abs(indexShift[i]) >= getBufferSize()(i)) {
        ...
        if (index + nCells <= getBufferSize()[i]) {
          // One region to drop.
          if (i == 0)
            resetCols(index, nCells);
          if (i == 1)
            resetRows(index, nCells);
        } else {
          // Two regions to drop.
          int firstIndex = index;
          int firstNCells = getBufferSize()[i] - firstIndex;
          if (i == 0)
            resetCols(firstIndex, firstNCells);
          if (i == 1)
            resetRows(firstIndex, firstNCells);

          int secondIndex = 0;
          int secondNCells = nCells - firstNCells;
          if (i == 0)
            resetCols(secondIndex, secondNCells);
          if (i == 1)
            resetRows(secondIndex, secondNCells);
        }
      }
    }
  }
  ...
  pose_.getPosition() += kindr::phys_quant::eigen_impl::Position3D(
      alignedPositionShift.x(), alignedPositionShift.y(), 0.0);

  if (indexShift.all() != 0)
    ROS_DEBUG("Elevation has been moved to position (%f, %f).",
              pose_.getPosition().x(), pose_.getPosition().y());
...
bool ElevationMap::getDataPointPositionInParentFrame(
  ...
  elevation_map_msg::getPositionFromIndex(positionInGrid, index, length_,
                                          position_.vector().head(2),
                                          resolution_, getBufferSize(),
                                          bufferStartIndex_);

  positionInParentFrame << positionInGrid.x(), positionInGrid.y(), height;
...
bool ElevationMap::clean() {
  varianceRawData_ = varianceRawData_.unaryExpr(
      VarianceClampOperator<double>(minVariance_, maxVariance_));
  horizontalVarianceRawDataX_ = horizontalVarianceRawDataX_.unaryExpr(
      VarianceClampOperator<double>(minHorizontalVariance_,
                                    maxHorizontalVariance_));
  horizontalVarianceRawDataY_ = horizontalVarianceRawDataY_.unaryExpr(
      VarianceClampOperator<double>(minHorizontalVariance_,
                                    maxHorizontalVariance_));
  return true;
}
