...
bool RobotMotionMapUpdater::update(
  ...
  if ((robotPoseCovariance - previousRobotPoseCovariance_).all() == 0)
    return false;
  ...
  // Covariance matrices.
  Matrix3d previousPositionCovariance = previousRobotPoseCovariance_
      .topLeftCorner<3, 3>();
  Matrix3d positionCovariance = robotPoseCovariance.topLeftCorner<3, 3>();
  Matrix3d previousRotationCovariance = previousRobotPoseCovariance_
  ...
  Matrix3d parentToMapRotation = RotationMatrixPD(map.getPose().getRotation())
      .matrix().transpose();
  ...
  kindr::phys_quant::eigen_impl::Position3D robotPosition = robotPose
      .getPosition();
      ...
      if (map.getDataPointPositionInParentFrame(Array2i(i, j), cellPosition)) {
        ...
        Matrix3d rotationJacobian = parentToMapRotation
            * kindr::linear_algebra::getSkewMatrixFromVector(
                (cellPosition - robotPosition).vector());
        ...
        varianceUpdate(i, j) = translationVarianceUpdate.z()
            + rotationVarianceUpdate.z();
        horizontalVarianceUpdateX(i, j) = translationVarianceUpdate.x()
            + rotationVarianceUpdate.x();
        horizontalVarianceUpdateY(i, j) = translationVarianceUpdate.y()
            + rotationVarianceUpdate.y();
      } else {
        // Cell invalid.
  ...
  map.update(varianceUpdate, horizontalVarianceUpdateX,
             horizontalVarianceUpdateY, time);
