...
#include <elevation_change_msgs/Toggle.h>
...
class ElevationChangeDetection
{
 ...
 private:
  ...
  /*!
   * ROS service callback function to toggle checking for unknown areas.
   * @param request the ROS service request.
   * @param response the ROS service response.
   * @return true if successful.
   */
  bool toggleCheckingUnknownAreas(
      elevation_change_msgs::Toggle::Request& request,
      elevation_change_msgs::Toggle::Response& response);
   ...
   * @return true if successful.
   */
  bool checkPolygonForUnknownAreas(
      const grid_map::Polygon& polygon,
      const geometry_msgs::PoseStamped& currentPose, grid_map::GridMap& map,
      std::vector<elevation_change_msgs::Obstacle>& obstacles);
   ...
   */
  bool getGridMap(const grid_map::Position& position,
                  const grid_map::Length& length, grid_map_msgs::GridMap& map);
  ...
  //! Toggle checking of unknown areas service server.
  ros::ServiceServer toggleCheckingUnknownAreasService_;
